...
document.getElementById("download_button").addEventListener("click", (e) => {
  getSliderValues();
  const name = "model_files/model_download/connectors/pulley"
  // const safeName = name.replace(/[^a-z0-9_-]/gi, "_");
  console.log(file_attributes);
  const link = document.getElementById("download_link");
  const fileName = `/pulley_${file_attributes[0]}_${file_attributes[1]}_${file_attributes[2]}__${file_attributes[3]}`;
